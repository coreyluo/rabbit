...
import com.bazinga.rabbit.enums.MarketTypeEnum;
...
public class MarketUtil {
    ...
    public static Integer getMarketCode(String stockCode) {
       ...
       if(stockCode.startsWith("688")){
        ...
        if(MarketUtil.isBj(stockCode)){
            return MarketTypeEnum.BJS.getCode();
        }
    ...
    public static boolean isAllMarketStocks(String stockCode) {
        if(StringUtils.isBlank(stockCode)){
            return false;
        }
        if(isShMain(stockCode)||isSzMain(stockCode)||isChuangYe(stockCode)||isKeChuang(stockCode)||isBj(stockCode)){
            return true;
        }
        return false;
    }
    ...
    public static boolean isBj(String stockCode) {
        if(StringUtils.isEmpty(stockCode)){
            return false;
        }
        if(stockCode.startsWith("83")||stockCode.startsWith("87")||stockCode.startsWith("43")||stockCode.startsWith("92")){
            return true;
        }
        return false;
    }
    ...
    public static String thsToGeneralStock(String thsStock){
        ...
        String result = general.replace(".CFE", "");
        return result;
    }
