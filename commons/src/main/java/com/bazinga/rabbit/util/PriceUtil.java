...
import com.bazinga.rabbit.constant.CommonConstant;
...
public class PriceUtil {
    ...
    public static boolean isHistoryUpperPrice(String stockCode,BigDecimal currentPrice, BigDecimal yesterdayPrice,String tradeDateStr) {
        ...
        if(!stockCode.startsWith("68")){
            if(date.before(DateUtil.parseDate("20200824",DateUtil.yyyyMMdd))){
                ...
                if (StringUtils.isNotBlank(stockCode) && stockCode.startsWith("3")) {
                    return currentPrice.compareTo(yesterdayPrice.multiply(CommonConstant.UPPER_RATE300).setScale(2, BigDecimal.ROUND_HALF_UP)) == 0;
                }
                return currentPrice.compareTo(yesterdayPrice.multiply(CommonConstant.UPPER_RATE).setScale(2, BigDecimal.ROUND_HALF_UP)) == 0;
            }
    ...
    public static boolean isHistorySuddenPrice(String stockCode,BigDecimal currentPrice, BigDecimal yesterdayPrice,String tradeDateStr) {
        ...
        if(!stockCode.startsWith("68")) {
            if (date.before(DateUtil.parseDate("20200824", DateUtil.yyyyMMdd))) {
                ...
                if (StringUtils.isNotBlank(stockCode) && (stockCode.startsWith("3") || stockCode.startsWith("68"))) {
    ...
    public static BigDecimal calHistoryUpperPrice(String stockCode,BigDecimal yesterdayPrice,String tradeDateStr) {
        Date date = DateUtil.parseDate(tradeDateStr, DateUtil.yyyyMMdd);
    ...
    public static BigDecimal calHistoryUpperPriceNew(String stockCode,BigDecimal yesterdayPrice,String tradeDateStr) {
        Date date = DateUtil.parseDate(tradeDateStr, DateUtil.yyyyMMdd);
        if(!stockCode.startsWith("68")){
            if(date.before(DateUtil.parseDate("20200824",DateUtil.yyyyMMdd))){
                return yesterdayPrice.multiply(CommonConstant.UPPER_RATE).setScale(2, BigDecimal.ROUND_HALF_UP);
     ...
     * @return  当前价格
     */
    public static BigDecimal absoluteRateToPriceFour(BigDecimal rate, BigDecimal yesterdayPrice) {
        BigDecimal mulRate = new BigDecimal("1").add(rate.multiply(new BigDecimal("0.01")).setScale(4, BigDecimal.ROUND_HALF_UP));
        return yesterdayPrice.multiply(mulRate).setScale(4,BigDecimal.ROUND_HALF_UP);
    }
